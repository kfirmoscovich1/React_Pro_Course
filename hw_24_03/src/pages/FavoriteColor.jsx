...
export default function FavoriteColor() {
  ...
  useEffect(() => {
    ...
    const savedColor = localStorage.getItem("favorite_color");
    if (savedColor) {
  ...
  const onSaveColor = () => {
    ...
    localStorage.setItem("favorite_color", color); // Save the color to localStorage
  };
  ...
  return (
    <div className='container' style={{ backgroundColor: bgColor, padding: "20px", minHeight: "100vh" }}>
      ...
      <button onClick={onSaveColor} className='btn btn-primary mt-2'>Save Color</button>
    </div>
  );
}
